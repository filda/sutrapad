...
- New source files are not done at "tests pass" — they're done when they're under mutation pressure too. Verify the file falls under one of the `mutate:` globs in `stryker.config.mjs`; if it doesn't, add it explicitly in the same change. Pure-data modules (frozen sets/maps, type-only files) get a matching `!` exclusion so they don't drag the score down with meaningless StringLiteral mutants. See [docs/development.md](docs/development.md#run-mutation-testing) for the current mutate scope and the rationale for what's deliberately excluded. `tests/mutate-scope.test.ts` enforces this — it fails on any `src/**/*.ts` that is neither mutated, `!`-excluded, nor listed in its `DEFERRED_FROM_MUTATION` map with a reason, and it also catches a mutated module whose only test `vi.mock`s it (how `view/palette.ts` sat at 0.00 % with 166 uncovered mutants).
- Notes can be body-less placeholders (`hydrated: false`, Phase 2 notes-scaling). Any code that reads a note's `body` must decide what a placeholder means for it — an empty body is "not loaded", never "the user wrote nothing". `tests/body-reader-scope.test.ts` fails on an unclassified reader; classify it there with a reason. Anything that writes a body or fans out over `workspace.notes` (Drive, proxies, storage) must respect the limits in `src/lib/budgets.ts`, and `tests/nfr/**` must stay green — see [docs/development.md](docs/development.md#non-functional-tests).
...
- User-facing UI strings are authored in English and live in the message
  ...
  [docs/i18n-plan.md](docs/i18n-plan.md) for what is done and what still
